...
### Optical Flow Sensor
The optical flow sensor is a sensor that the CoDrone EDU and the CoDrone EDU (JROTC edition) use to calculate their position. It is located in the center of the bottom of the drone. WIth the help of the optical flow sensor, the user can send commands to the drone to move a given distance or move to a given position, or the user can retrieve position data from the drone.

<hr/>

### Drone Coordinate System
The optical flow sensor is responsible for setting up or initializing the drone's coordinate system. When the drone's coordinate system is initialized:
- the x-axis points forward from the drone
- the y-axis points left from the drone
- z-axis points vertically upward
- the position data is set to (x, y, z) = (0, 0, 0)
- the gyroscope data, which includes the x- (roll), y- (pitch), and z-angle (yaw), is also set to 0
<img src="/img/CDE/python_docu/xyz.jpg"  width="400px"/>
The drone's coordinate system is initialized or reset when the user inserts the battery into the drone or when the drone receives a takeoff command.

:::important
Simply re-running code or using ``pair()`` does **not** reset the drone's coordinate system, position data, and gyroscope data. Unless the drone takes off or battery is re-inserted before retrieving position or gyroscope data, the drone will continue to retrieve position and gyroscope data based off of the coordinate system from the most recent takeoff or battery insertion.
:::
...
drone.move_distance(-0.75, 0, 0, 0.75) # move right 0.75m at 0.75m/d
drone.land()
...
* data[29] battery percentage level
